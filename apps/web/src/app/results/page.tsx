...
export default async function ResultsPage({
  ...
  if (mode === "given") {
    ...
    const [{ data: aggData }, { data: txtData }, { data: vmData }, { data: sumData }] = await Promise.all([
      ...
      supabase.from("result_summaries").select("ai_summary,theme_tags,structured_output").eq("cycle_id", CYCLE).eq("recipient_id", target).eq("scope", "rag_full").eq("review_status", "ready").maybeSingle(),
    ]);
    ...
    summary = sumData as AiSummary | null;
  } else {
          ...
          <>
            ...
            {slice === "all" && summary?.ai_summary && (
              <div className="mb-6 space-y-3">
                {/* Header + narrative */}
                <Card className="bg-mint-light ring-mint">
                  <h2 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-aqua-700">
                    <Icon name="sparkles" size={16} />
                    {cs ? "AI shrnutí zpětné vazby" : "AI feedback summary"}
                  </h2>
                  <p className="text-sm leading-relaxed text-ink">{summary.ai_summary}</p>
                  {summary.theme_tags && summary.theme_tags.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {summary.theme_tags.map((tag) => (
                        <Badge key={tag} tone="aqua">{tag}</Badge>
                      ))}
                    </div>
                  )}
                </Card>
                ...
                {/* Strengths */}
                {summary.structured_output?.strengths && summary.structured_output.strengths.length > 0 && (
                  <Card>
                    <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-ink">
                      <Icon name="check" size={15} className="text-aqua-700" />
                      {cs ? "Silné stránky" : "Strengths"}
                    </h3>
                    <ul className="space-y-1.5">
                      {summary.structured_output.strengths.map((s, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-ink">
                          <span className="mt-0.5 shrink-0 text-aqua-700">✓</span>
                          <span>{s}</span>
                        </li>
                      ))}
                    </ul>
                ...
                {/* Growth areas */}
                {summary.structured_output?.growth_areas && summary.structured_output.growth_areas.length > 0 && (
                  <Card>
                    <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-ink">
                      <Icon name="chevronRight" size={15} className="text-amber-600" />
                      {cs ? "Oblasti pro rozvoj" : "Growth areas"}
                    </h3>
                    <ul className="space-y-1.5">
                      {summary.structured_output.growth_areas.map((g, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-ink">
                ...
                {/* Polarizing traits */}
                {summary.structured_output?.polarizing_traits && summary.structured_output.polarizing_traits.length > 0 && (
                  <Card className="ring-amber-200">
                    <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-ink">
                      <Icon name="info" size={15} className="text-amber-600" />
                      {cs ? "Rozdělující vlastnosti" : "Polarizing traits"}
                    </h3>
                    <p className="mb-2 text-xs text-ink-600">
                      {cs
                        ? "Tyto vlastnosti vnímají různí respondenti odlišně."
                        : "These traits were perceived differently by different respondents."}
                    </p>
                    <ul className="space-y-1.5">
                      {summary.structured_output.polarizing_traits.map((p, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-ink">
                          <span className="mt-0.5 shrink-0 text-amber-600">⇄</span>
                          <span>{p}</span>
                        </li>
